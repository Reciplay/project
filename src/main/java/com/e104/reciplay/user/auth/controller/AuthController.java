...
public class AuthController {
    ...
    public ResponseEntity<ResponseRoot<List<String>>> querySimillarEmails(
        ...
        return CommonResponseBuilder.success("이메일 조회에 성공했습니다.", List.of("더미데이터"));
    }
    ...
    @Operation(summary = "이메일 인증번호 인증", description = "발송된 인증 번호를 인증함.")
    @ApiResponse(responseCode = "200", description = "인증 성공, 가입을 위한 일회성 토큰을 제공함.")
    @ApiResponse(responseCode = "403", description = "인증 실패")
    public ResponseEntity<ResponseRoot<EmailHash>> verifyEmailOTP(
            @RequestParam("email") String email,
            @RequestParam("otp") String otp
    ) {

        return CommonResponseBuilder.success("이메일 인증에 성공 했습니다.", new EmailHash());
    }
    ...
    public ResponseEntity<ResponseRoot<Boolean>> checkDuplicatedEmail(
        ...
        return CommonResponseBuilder.success("조회 결과입니다.", true);
    }
