...
export default function CategoryForm({ label }: Props) {
  const categoryId = useCreateCourseStore(
    (s) => s.values.requestCourseInfo.categoryId,
  );
  const error = useCreateCourseStore(
    (s) => s.errors["requestCourseInfo.categoryId"],
  );
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <Radio.Group
        ...
        onChange={(e) =>
          setField("requestCourseInfo.categoryId", Number(e.target.value))
        }
        style={{ marginTop: 8 }}
      >
        {CATEGORIES.map((c) => (
