          ...
          <div key={e.id} className={styles.imageSlide}>
            ...
            <Image
              ...
              style={{
                ...
                ...(idx === 1 ? { left: "28%", width: "29%" } : {}) // ← 두 번째만 왼쪽으로 이동
              }}
            />

            {/* 텍스트 오버레이 (항상 최상단!) */}
            <div
              className={`${styles.textOverlay} ${e.isItemRight ? styles.leftAlign : styles.rightAlign
                } ${idx === 1 ? `${styles.glassOverlay} ${styles.variantSmallText} ${styles.glassSoft}` : ""}`}
            >
              <div className={styles.title}>
                {idx === 1 ? e.title.replace(", ", ",\n") : e.title}
              </div>
              <div className={styles.desc}>{e.desc}</div>
              <div className={styles.slogan}>{e.slogan}</div>
              <a className={styles.ctaButton} href="#">바로가기</a>
            </div>
          </div>
        ))}
      </AntdCarousel>
